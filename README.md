...
데모 녹화본(`SafePrompt_demo.mov`, 약 109MB)은 **GitHub 단일 파일 100MB 제한** 때문에 이 저장소에는 포함하지 않습니다. 아래 링크에서 전체 흐름(질문 입력 → 위험 분석 → 의도 확인 → 프롬프트 교정 → 답변)을 확인할 수 있습니다.

| | |
|---|---|
| **데모 영상** | *(업로드 후 링크를 여기에 추가)* |
| **로컬 파일** | 저장소 루트 `SafePrompt_demo.mov` (Git 미추적, `.gitignore`에 `*.mov` 등록) |

**링크 올리는 방법 (택 1)**

- **GitHub Releases** — 저장소 **Releases → Draft a new release**에서 `SafePrompt_demo.mov`를 첨부한 뒤, 생성된 다운로드 URL을 위 표에 넣습니다.
- **YouTube / Google Drive** — 비공개·일부 공개 업로드 후 공유 링크를 README에 붙입니다.

링크를 추가한 뒤 이 섹션의 `*(업로드 후 링크를 여기에 추가)*` 부분만 실제 URL로 바꾸면 됩니다.
...
7. [실행](#실행)
8. [API 개요](#api-개요)
9. [프론트 빌드·클라우드 배포](#프론트-빌드클라우드-배포)
10. [프로젝트 구조](#프로젝트-구조)
11. [기타 스크립트](#기타-스크립트)
12. [문제 해결](#문제-해결)
13. [라이선스](#라이선스)
