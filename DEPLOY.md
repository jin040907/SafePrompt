...
2. **Configure Project** 화면에서:
   - **Root Directory** → **Edit** → `web` 입력 후 저장  
     (이미 만든 프로젝트면 **Settings → General → Root Directory**)
   - **Framework Preset**: Vite 로 자동 인식되면 그대로 두면 됩니다.
...
4. **Settings → General** 에서 **Output Directory** 가 비어 있거나 `dist` 인지 확인합니다.  
   예전에 `dist`만 찾는 오류가 났다면, 루트 기준으로 잘못 저장된 값이 있을 수 있으니 **비우기** 후 다시 배포해 보세요.
...
| `Invalid value for '--port': '$PORT'` | Railway **Start Command**에 `uvicorn ... --port $PORT` 를 넣으면 셸이 없어 `$PORT` 가 문자 그대로 전달될 수 있음. **Start Command 비우기**(Dockerfile `CMD` 또는 `Procfile` 사용). 시작은 루트 `serve.py`가 `os.environ["PORT"]`로 읽음 |
...
| `No Output Directory named "dist"` | **Root Directory** 를 `web` 으로 두고 (`web/vercel.json` 사용), Output Directory 는 `dist` 또는 비움. 저장소 루트만 쓰는 설정과 섞이면 산출물 경로가 어긋날 수 있음. |
